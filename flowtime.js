...
var FlowtimeNew = (function ()
{
  ...
  var _crossDirection = Brav1ToolboxNew.hasClass(ftContainer, CROSS_DIRECTION_CLASS);       // flag to set the cross direction layout and logic
  var _transformProperty = Brav1ToolboxNew.getPrefixed("transform");
  var _supportsTransform = Brav1ToolboxNew.testCSS("transform");
  ...
  var _scrollTheSection = Brav1ToolboxNew.hasClass(ftContainer, SCROLL_THE_SECTION_CLASS);  // flag to set the scroll the section logic
  var _sectionsStatus = [];
  ...
  (function initTransitionTime() {
    var tt = Brav1ToolboxNew.getCSSValue(ftContainer, "transitionDuration");
   ...
   */
  if (browserSupport) {
    Brav1ToolboxNew.addClass(ftParent, "ft-absolute-nav");
  }
 ...
 #######  ##        ########  ##     ##    ##    ########  #######  ##       ##        ######  ########    ##     ######  
    */
    ...
    function _showFragment(fp, fsp, f) {
      ...
      //
      for (var i = 0; i <= f; i++) {
        Brav1ToolboxNew.addClass(fragmentsArray[fp][fsp][i], FRAGMENT_REVEALED_CLASS);
        Brav1ToolboxNew.removeClass(fragmentsArray[fp][fsp][i], FRAGMENT_ACTUAL_CLASS);
        if (_fragmentsAlwaysOnOnceRevealed === true) {
          Brav1ToolboxNew.addClass(fragmentsArray[fp][fsp][i], FRAGMENT_REVEALED_ONCE_CLASS);
        }
      }
      Brav1ToolboxNew.addClass(currentFragment, FRAGMENT_ACTUAL_CLASS);
    }
    ...
    function _hideFragment(fp, fsp, f) {
      ...
      for (var i = 0; i < fragmentsArray[fp][fsp].length; i++) {
          if (i >= f) {
            Brav1ToolboxNew.removeClass(fragmentsArray[fp][fsp][i], FRAGMENT_REVEALED_CLASS);
            Brav1ToolboxNew.removeClass(fragmentsArray[fp][fsp][i], FRAGMENT_REVEALED_TEMP_CLASS);
          }
          Brav1ToolboxNew.removeClass(fragmentsArray[fp][fsp][i], FRAGMENT_ACTUAL_CLASS);
      }
      f -= 1;
      if (f >= 0) {
        Brav1ToolboxNew.addClass(fragmentsArray[fp][fsp][f], FRAGMENT_ACTUAL_CLASS);
      }
      fr[fp][fsp] = f;
    }
    ...
    function _showFragments() {
      for (var i = 0; i < fragments.length; i++) {
        Brav1ToolboxNew.addClass(fragments[i], FRAGMENT_REVEALED_TEMP_CLASS);
    ...
    function _hideFragments() {
      for (var i = 0; i < fragments.length; i++) {
        Brav1ToolboxNew.removeClass(fragments[i], FRAGMENT_REVEALED_TEMP_CLASS);
      }
    }
    ...
    function _switchActivePage(d, navigate) {
      ...
      for (var i = 0; i < sectionsArray.length; i++) {
        ...
        for (var ii = 0; ii < pa.length; ii++) {
          var spa = pa[ii];
          //
          Brav1ToolboxNew.removeClass(spa, "past-section");
          Brav1ToolboxNew.removeClass(spa, "future-section");
          Brav1ToolboxNew.removeClass(spa, "past-page");
          Brav1ToolboxNew.removeClass(spa, "future-page");
          //
          if (spa !== d) {
            Brav1ToolboxNew.removeClass(spa, "hilite");
            if (_isOverview === false && spa !== _getCurrentPage()) {
              Brav1ToolboxNew.removeClass(spa, "actual");
            }
            if (i < sIndex) {
              Brav1ToolboxNew.addClass(spa, "past-section");
            } else if (i > sIndex) {
              Brav1ToolboxNew.addClass(spa, "future-section");
            }
            if (spa.index < d.index) {
              Brav1ToolboxNew.addClass(spa, "past-page");
            } else if (spa.index > d.index) {
              Brav1ToolboxNew.addClass(spa, "future-page");
            }
          }
        }
      }
      Brav1ToolboxNew.addClass(d, "hilite");
      if (navigate) {
        setActual(d);
      }
      hilited = d;
    ...
    function setActual(d) {
      Brav1ToolboxNew.addClass(d, "actual");
    }
...
*/
  ...
  if (browserSupport) {
    if (isTouchDevice) {
      Brav1ToolboxNew.addListener(document, "touchend", function(e) {
        // e.preventDefault(); // TODO FIX
        onNavClick(e);
      }, false);
    }
    Brav1ToolboxNew.addListener(document, "click", onNavClick, false);
  }
  ...
  function onNavClick(e) {
    if (_areLinksActive) {
      ...
      if (_isOverview) {
        var target = e.target;
        while (target && !Brav1ToolboxNew.hasClass(target, PAGE_CLASS)) {
          target = target.parentNode;
        }
        if (Brav1ToolboxNew.hasClass(target, PAGE_CLASS)) {
          e.preventDefault();
          navigateTo(target, null, true);
        }
      }
      // thumbs in the default progress indicator
      if (Brav1ToolboxNew.hasClass(e.target, PAGE_THUMB_CLASS)) {
        e.preventDefault();
   ...
   */
  Brav1ToolboxNew.addListener(window, "hashchange", onHashChange);
  //
  /**
...
*/
  ...
  function _setMouseDrag(value) {
    ...
    if (_mouseDragEnabled) {
      Brav1ToolboxNew.addListener(ftContainer, "mousedown", onTouchStart, false);
      Brav1ToolboxNew.addListener(ftContainer, "mouseup", onTouchEnd, false);
    } else {
      Brav1ToolboxNew.removeListener(ftContainer, "mousedown", onTouchStart);
      Brav1ToolboxNew.removeListener(ftContainer, "mouseup", onTouchEnd);
    }
  }
...
*/
  ...
  function onTouchStart(e) {
    ...
    _ftY = initOffset.y;
    if (_mouseDragEnabled) {
      Brav1ToolboxNew.addListener(ftContainer, "mousemove", onTouchMove, false);
    }
  }
  ...
  function onTouchEnd(e) {
    ...
    Brav1ToolboxNew.removeListener(ftContainer, "mousemove", onTouchMove);
  }
...
*/
   ...
   */
  Brav1ToolboxNew.addListener(window, "scroll", onNativeScroll);
...
*/
  ...
  var resizeMonitor = (function _resizeMonitor() {
    ...
    Brav1ToolboxNew.addListener(window, "resize", _enable);
...
*/
  ...
  function fireNavigationEvent() {
    if (_fireEvent !== false) {
      ...
      Brav1ToolboxNew.dispatchEvent(NAVIGATION_EVENT, eventData);
      //
  ...
  function buildProgressIndicator() {
    ...
    for (var i = 0; i < NavigationMatrix.getSectionsLength(); i++) {
      var pDiv = document.createElement("div");
        ...
        Brav1ToolboxNew.addClass(pDiv, "thumb-section-" + i);
      // loop through pages
      ...
      for (var ii = 0; ii < spArray.length; ii++) {
        var spDiv = document.createElement("div");
          ...
          Brav1ToolboxNew.addClass(spDiv, "thumb-page-" + ii);
    ...
    if (defaultProgress !== null) {
      ...
      for (var i = 0; i < spts.length; i++) {
        ...
        if (pTo == NavigationMatrix.getPageIndex().section && spTo == NavigationMatrix.getPageIndex().page) {
          Brav1ToolboxNew.addClass(spts[i], "actual");
        } else {
          Brav1ToolboxNew.removeClass(spts[i], "actual");
        }
      }
...
*/
  ...
  function zoomIn(back, navigate) {
    _isOverview = false;
    Brav1ToolboxNew.removeClass(body, "ft-overview");
  ...
  function zoomOut() {
    _isOverview = true;
    Brav1ToolboxNew.addClass(body, "ft-overview");
...
*/
   ...
   * KEYBOARD NAVIGATION
   */
  Brav1ToolboxNew.addListener(window, "keydown", onKeyDown);
  Brav1ToolboxNew.addListener(window, "keyup", onKeyUp);
...
*/
  ...
  function _pauseTransitions(restoreAfter) {
    ...
    ftContainer.style[Brav1ToolboxNew.getPrefixed("transition-duration")] = "0ms";
  ...
  function _restoreTransitions(withTransitionDelay) {
    ...
    if (withTransitionDelay === true) {
      setTimeout(function() {
        ftContainer.style[Brav1ToolboxNew.getPrefixed("transition-duration")] = "" + _transitionTime / 1000 + "s";
      }, _transitionTime);
    } else {
      ftContainer.style[Brav1ToolboxNew.getPrefixed("transition-duration")] = "" + _transitionTime / 1000 + "s";
    }
  ...
  function _gotoPage() {
    ...
    if (args.length > 0) {
      var spd = null;
      if (args.length == 1) {
        if (Brav1ToolboxNew.typeOf(args[0]) === "Object") {
          var o = args[0];
          var p = o.section;
          var sp = o.page;
      ...
      if (Brav1ToolboxNew.typeOf(args[0]) === "Number" || args[0] === 0) {
  ...
  function _addEventListener(type, handler, useCapture) {
    Brav1ToolboxNew.addListener(document, type, handler, useCapture);
  }
...
*/
  ...
  function _setCrossDirection(v) {
    if (_crossDirection !== v) {
      ...
      if (!Brav1ToolboxNew.hasClass(ftContainer, CROSS_DIRECTION_CLASS) && _crossDirection === true) {
        Brav1ToolboxNew.addClass(ftContainer, CROSS_DIRECTION_CLASS);
      } else if (Brav1ToolboxNew.hasClass(ftContainer, CROSS_DIRECTION_CLASS) && _crossDirection !== true) {
        Brav1ToolboxNew.removeClass(ftContainer, CROSS_DIRECTION_CLASS);
      }
      if (defaultProgress) {
        if (!Brav1ToolboxNew.hasClass(defaultProgress, CROSS_DIRECTION_CLASS) && _crossDirection === true) {
          Brav1ToolboxNew.addClass(defaultProgress, CROSS_DIRECTION_CLASS);
        } else if (Brav1ToolboxNew.hasClass(defaultProgress, CROSS_DIRECTION_CLASS) && _crossDirection !== true) {
          Brav1ToolboxNew.removeClass(defaultProgress, CROSS_DIRECTION_CLASS);
        }
      }
      //
      NavigationMatrix.updateOffsets();
  ...
  function _setScrollTheSection(v) {
    if (_scrollTheSection !== v) {
      ...
      if (!Brav1ToolboxNew.hasClass(ftContainer, SCROLL_THE_SECTION_CLASS) && _scrollTheSection === true) {
        Brav1ToolboxNew.addClass(ftContainer, SCROLL_THE_SECTION_CLASS);
      } else if (Brav1ToolboxNew.hasClass(ftContainer, SCROLL_THE_SECTION_CLASS) && _scrollTheSection !== true) {
        Brav1ToolboxNew.removeClass(ftContainer, SCROLL_THE_SECTION_CLASS);
      }
      //
      NavigationMatrix.updateOffsets();
      navigateTo();
  ...
  function _setTransitionTime(milliseconds) {
    ...
    ftContainer.style[Brav1ToolboxNew.getPrefixed("transition-duration")] = "" + _transitionTime + "ms";
  }
